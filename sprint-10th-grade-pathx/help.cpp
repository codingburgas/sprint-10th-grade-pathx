...
void ShowHelp() {
    ...
    while (running && !WindowShouldClose()) {
        ...
        DrawText(section1, screenWidth / 2 - MeasureText(section1, 28) / 2, 120, 28, WHITE);

        const char* objective = "Find your way out of the maze using the WASD keys.";
        DrawText(objective, screenWidth / 2 - MeasureText(objective, fontSizeText) / 2, 160, fontSizeText, LIGHTGRAY);

        DrawText(section2, screenWidth / 2 - MeasureText(section2, 28) / 2, 220, 28, WHITE);
        ...
        for (int i = 0; i < 5; i++) {
            ...
            DrawText(controls[i], screenWidth / 2 - textWidth / 2, 260 + i * 30, fontSizeText, LIGHTGRAY);
        }

        DrawText(section3, screenWidth / 2 - MeasureText(section3, 28) / 2, 440, 28, WHITE);
        ...
        for (int i = 0; i < 3; i++) {
            ...
            DrawText(levels[i], screenWidth / 2 - textWidth / 2, 480 + i * 30, fontSizeText, LIGHTGRAY);
        }
        EndDrawing();
