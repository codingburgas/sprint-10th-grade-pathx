...
static void PlaceDoorsAndItems() {
    maze[3][3].hasDoor = true; maze[3][3].doorColor = 1; // red door
...
static void DrawMaze(int cellSize, int ox, int oy) {
    for (int y = 0; y < GRID_H; y++)
        for (int x = 0; x < GRID_W; x++) {
            ...
            if (dx + dy <= player.visionRadius) DrawCell(x, y, cellSize, ox, oy);
            else DrawRectangle(ox + x * cellSize, oy + y * cellSize, cellSize, cellSize, BLACK);
        }
...
void StartGameDoorMaze() {
    ...
    while (!WindowShouldClose()) {
        ...
        if (maze[player.y][player.x].hasDoor) {
            ...
            if (!player.keys[color - 1]) { player.x = oldX; player.y = oldY; }
        }
