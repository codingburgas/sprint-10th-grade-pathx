...
static Cell** CreateGrid(int w, int h) {
    ...
    for (int x = 0; x < w; ++x) {
        ...
        for (int y = 0; y < h; ++y) {
            g[x][y].visited = false;
            g[x][y].top = g[x][y].bottom = g[x][y].left = g[x][y].right = true;
        }
    }
    return g;
}
...
// Добавя допълнителни връзки, за да стане лабиринтът "по-лесен" (повече разклонения)
static void AddExtraConnections(Cell** g, int w, int h, int extraCount) {
    for (int i = 0; i < extraCount; ++i) {
        ...
        if (dir == 3) nx--;
        if (nx < 0 || nx >= w || ny < 0 || ny >= h) continue;
        if (dir == 0) { g[x][y].top = false;    g[nx][ny].bottom = false; }
        if (dir == 1) { g[x][y].right = false;  g[nx][ny].left = false; }
        if (dir == 2) { g[x][y].bottom = false; g[nx][ny].top = false; }
        if (dir == 3) { g[x][y].left = false;   g[nx][ny].right = false; }
    }
}
...
void StartEasyGame() {
    // Предполага, че прозорецът вече е инициализиран от main (fullscreen)
    Cell** maze = CreateGrid(GRID_W, GRID_H);
    Generate(maze, 0, 0, GRID_W, GRID_H);
    // повече връзки -> по-лесно
    AddExtraConnections(maze, GRID_W, GRID_H, (GRID_W * GRID_H) / 6);
    ...
    while (!WindowShouldClose()) {
        ...
        // рисуваме целия лабиринт (за easy няма тъмнини)
    ...
    // връщаме се в менюто (main поддържа прозореца)
}
