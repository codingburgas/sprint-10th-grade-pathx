...
void StartMediumGame() {
    ...
    float nextFlash = (float)GetRandomValue(5, 6); // време до следващо "премигване"
    bool flashNow = false;
    ...
    while (!WindowShouldClose()) {
        ...
        if (flashTimer >= nextFlash) {
            flashNow = true;
            flashTimer = 0.0f;
            nextFlash = (float)GetRandomValue(5, 6);
        }
        ...
        if (flashNow) {
            // еднократно показване на целия лабиринт
            ...
            // нормално: цялото + силно затъмнение + видим прозорец около играча
            DrawRegion(maze, GRID_W, GRID_H, cellSize, ox, oy, 0, 0, GRID_W, GRID_H);
            DrawRectangle(0, 0, sw, sh, Fade(BLACK, 0.90f)); // тъмнина
            int radiusCells = 3;
            int sx = px - radiusCells;
            int sy = py - radiusCells;
            int ex = px + radiusCells + 1;
            int ey = py + radiusCells + 1;
